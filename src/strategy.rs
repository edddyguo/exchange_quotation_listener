...
use crate::{get_last_bar_shape_score, get_last_bar_volume_score, get_raise_bar_num, is_break_through_market, Kline, KLINE_NUM_FOR_FIND_SIGNAL, MathOperation, MathOperation2, notify_lark, recent_kline_shape_score, Symbol, take_order, TakeOrderInfo, timestamp2date};
...
pub async fn sell(take_order_pair2: &mut HashMap<String,TakeOrderInfo>,
    ...
    if is_break_through_market(pair_symbol, &line_datas).await {
        ...
        info!("------: market {},shape_score {},volume_score {},recent_shape_score {}",
        ...
        if take_order_pair2.get(pair_symbol).is_none() && shape_score >= 4 && volume_score >= 3 && recent_shape_score >= 5 {
            ...
            warn!("{},now {},bar {:?}",push_text,timestamp2date(now),line_datas[19]);
            if is_real_trading {
    ...
    match take_order_pair2.get(pair_symbol) {
        ...
        Some(take_info) => {
            if take_info.is_took == true {
                let price_raise_ratio = line_datas[KLINE_NUM_FOR_FIND_SIGNAL - 1].open_price.to_f32() / take_info.price;
                //20X情况下：0.4个点止损,高峰之后根据20根k线之后，价格是否大于10根之前的价格5次这种情况就止盈
                if price_raise_ratio > 1.05
                    || (line_datas[KLINE_NUM_FOR_FIND_SIGNAL - 20].open_time > take_info.take_time && get_raise_bar_num(&line_datas[KLINE_NUM_FOR_FIND_SIGNAL - 20..]) >= 6) {
                    ...
                    if is_real_trading {
                        ...
                        warn!("{} ,now {}, {:?}",push_text,timestamp2date(now),line_datas[KLINE_NUM_FOR_FIND_SIGNAL - 1]);
                    }
                    take_order_pair2.remove(pair_symbol);
                    return Ok(true);
